...
const ColorList = ({ setSaveForm, saveForm }) => {
    ...
    const colorTemplate = (color, key) => {
        const btn_color = `bg-${color.bg} text-${color.text}`
        if (color.bg === saveForm.color.bg && color.text === saveForm.color.text)
            return <button key={key}>
                <div className={`${btn_color} bg-opacity-25 h-10 p-0.5 flex items-center justify-center active:gray-100`}>
                    <span className={`${btn_color} w-full h-full flex items-center justify-center`}>
                        <FontAwesomeIcon icon="fa-solid fa-bookmark" />
                    </span>
                </div>
            </button>
        else
            return <button onClick={() => setSaveForm({ ...saveForm, color: { bg: color.bg, text: color.text } })} key={key}>
                <div className={`${btn_color} h-10 text-sm flex items-center justify-center active:gray-100`}>
